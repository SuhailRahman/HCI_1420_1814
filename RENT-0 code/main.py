...
def index():
	if request.method == 'GET':
		...
		top_products=sorted(top_prod, key = lambda x: x[4])
		cur.close()
...
def product1():
	if request.method == 'GET':
		...
		limit=6-length
		sql="select name,image1,description,price,product_id from product where category=%s LIMIT %s"
		val=(str(category[0][0]),limit)
		cur.execute(sql,val)
		r_prod=cur.fetchall()
